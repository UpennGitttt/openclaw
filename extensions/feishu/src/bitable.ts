...
export function registerFeishuBitableTools(api: OpenClawPluginApi) {
  // Bitable uses resolveFeishuToolAccount directly (not resolveFeishuToolContext) because
  // bitable tools have no independent per-tool toggle in FeishuToolsConfig — they are
  // available whenever a valid Feishu account is configured.
  if (!api.config || !resolveFeishuToolAccount({ cfg: api.config })) {
  ...
  api.registerTool(
    (ctx) => {
      const account = resolveFeishuToolAccount({ cfg: api.config, ctx });
      if (!account) {
        return null;
      }
